...
export default function ConfiguracionPage() {
  ...
  // Menu Digital: la pantalla vive en /admin/menu-qr. Aqui solo se lee el worker_url
  // para ofrecerlo como atajo en la URL base de los QR de mesa (Self Order).
  ...
  // Worker URL del menu digital (atajo para la URL base de los QR de mesa)
  useEffect(() => {
    if (!selected?.id) { setMdWorkerUrl(''); return; }
    menuDigitalApi.getStatus(selected.id)
      .then(({ data }) => setMdWorkerUrl(data?.config?.worker_url || ''))
      .catch(() => setMdWorkerUrl(''));
  }, [selected?.id]);
  ...
  const selectTienda = (tienda: any) => {
    ...
    setForm({
      ...
      cliente_venta_enabled: cp.cliente_venta_enabled || false,
    ...
    try {
      const payload: any = {
        ...
        config_pos: {
          ...
          cliente_venta_enabled: form.cliente_venta_enabled,
    ...
    <div className="p-4 max-w-6xl mx-auto">
      ...
      {configTab === 'tienda' && <div>
        ...
        <div className="space-y-2">
          ...
          {/* El Menu Digital QR se movio a su propia pantalla (sidebar > Menu QR):
              es un modulo de cara al cliente y aqui, plegado entre impresoras y
              pasarelas, pasaba desapercibido. */}
          {selected && (
            <>
              ...
              {expandedSection === 'menu-digital' && (
                <div className="card">
                  <p className="text-xs text-slate-400 mb-3">
                    Esta configuracion ahora vive en su propia pantalla, con mas espacio: publicar el menu,
                    imprimir el QR y atender los pedidos que entran por el.
                  </p>
                  <button
                    onClick={() => navigate('/admin/menu-qr')}
                    className="btn-primary text-xs flex items-center gap-1"
                  >
                    <QrCode size={14} /> Ir a Menu QR
                  </button>
                </div>
              )}
            </>
                  ...
                  {bsConfig?.tienda_token && (
                    <div className="border-t border-iados-card pt-4">
                      <div className="flex items-center justify-between mb-1">
                        <label className="text-xs text-slate-400 flex items-center gap-1"><Key size={11} /> Token del bridge (se pega en la ventana Configuracion del bridge)</label>
                  ...
                  <div className="border-t border-iados-card pt-4 space-y-3">
                    ...
                    <div className="flex items-center gap-3 flex-wrap pt-1">
                      ...
                      <a href={resolveUploadUrl('/api/uploads/downloads/bascula-bridge.zip')} download
                        className="flex items-center gap-1 text-xs text-slate-500 hover:text-slate-300 border border-slate-700 rounded-lg px-3 py-1.5">
                        <Download size={12} /> Version fuente (Node, avanzado)
                      </a>
                    </div>
                    <p className="text-xs text-slate-600">
                      Probado con basculas Torrey serie L-PCR / PCR con salida serial/USB, incluidas las que solo responden
                      cuando se les pregunta el peso. Para otras marcas, "Detectar automaticamente" prueba los comandos mas
                      comunes; si ninguno funciona se puede ajustar a mano en la misma ventana (ver <code className="bg-slate-700 px-1 rounded text-blue-300">bascula-bridge/LEEME.txt</code>).
                    </p>
                  </div>
                ...
                <div className="card space-y-3">
                  ...
                  <div className="border-t border-slate-700 pt-3 mt-3">
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={form.precuenta_enabled}
                        onChange={(e) => setForm({ ...form, precuenta_enabled: e.target.checked })}
                        className="w-5 h-5 accent-iados-primary rounded"
                      />
                      <div>
                        <span className="text-sm font-medium">Habilitar Pre-cuenta</span>
                        <p className="text-xs text-slate-500">Muestra botón para imprimir pre-cuenta en carrito, cuentas abiertas y pedidos</p>
                      </div>
                    </label>
                  </div>
                  ...
                  {/* Enlazar cliente por teléfono en la venta (opcional) */}
                  <div className="border-t border-slate-700 pt-3 mt-3">
                    <label className="flex items-center gap-3 cursor-pointer mb-2">
                      <input
                        type="checkbox"
                        checked={form.cliente_venta_enabled}
                        onChange={(e) => setForm({ ...form, cliente_venta_enabled: e.target.checked })}
                        className="w-5 h-5 accent-iados-primary rounded"
                      />
                      <div>
                        <span className="text-sm font-medium">Enlazar cliente por teléfono en la venta</span>
                        <p className="text-xs text-slate-500">Permite capturar (opcionalmente, no obligatorio) el teléfono/nombre del cliente al cobrar en el POS, con autocompletado de clientes ya registrados</p>
                      </div>
                    </label>
                  </div>
                          ...
                          <div>
                            ...
                            {form.self_order_enabled && (
                              <div className="mt-3 space-y-1">
                                ...
                                {mdWorkerUrl && !form.self_order_url && (
                                  <button
                                    type="button"
                                    onClick={() => setForm({ ...form, self_order_url: mdWorkerUrl })}
