...
async function seed() {
    ...
    const [tenant] = await ds.query(`INSERT INTO tenants (nombre, slug, razon_social, email) VALUES (?, ?, ?, ?)
     ON DUPLICATE KEY UPDATE nombre=nombre`, ['iaDoS Corp', 'iados-corp', 'iaDoS - Inteligencia Artificial DevOps Solutions', 'info@iados.mx']);
    const tenantId = tenant.insertId || 1;
    const [empresa] = await ds.query(`INSERT INTO empresas (tenant_id, nombre, razon_social) VALUES (?, ?, ?)
     ON DUPLICATE KEY UPDATE nombre=nombre`, [tenantId, 'Restaurante Demo iaDoS', 'Restaurante Demo SA de CV']);
    const empresaId = empresa.insertId || 1;
    const [tienda] = await ds.query(`INSERT INTO tiendas (tenant_id, empresa_id, nombre, direccion) VALUES (?, ?, ?, ?)
     ON DUPLICATE KEY UPDATE nombre=nombre`, [tenantId, empresaId, 'Sucursal Centro', 'Av. Principal #123, Centro']);
    const tiendaId = tienda.insertId || 1;
    const passHash = await bcrypt.hash('admin123', 10);
    await ds.query(`INSERT INTO users (tenant_id, empresa_id, tienda_id, nombre, email, password, rol, pin)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)
     ON DUPLICATE KEY UPDATE nombre=nombre`, [tenantId, empresaId, tiendaId, 'Super Admin', 'admin@iados.mx', passHash, 'superadmin', '0000']);
    const cajeroPass = await bcrypt.hash('cajero123', 10);
    await ds.query(`INSERT INTO users (tenant_id, empresa_id, tienda_id, nombre, email, password, rol, pin)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)
     ON DUPLICATE KEY UPDATE nombre=nombre`, [tenantId, empresaId, tiendaId, 'Cajero Demo', 'cajero@iados.mx', cajeroPass, 'cajero', '1234']);
    const categorias = [
    ...
    for (const cat of categorias) {
        await ds.query(`INSERT INTO categorias (tenant_id, empresa_id, nombre, color, icono, orden, es_seccion_especial, tipo_seccion)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE nombre=nombre`, cat);
    }
    const productos = [
    ...
    for (const prod of productos) {
        await ds.query(`INSERT INTO productos (tenant_id, empresa_id, sku, nombre, descripcion, precio, costo, categoria_id, unidad, impuesto_pct)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pza', ?) ON DUPLICATE KEY UPDATE nombre=VALUES(nombre)`, [tenantId, empresaId, ...prod]);
    }
    await ds.query(`INSERT INTO ticket_configs (tenant_id, encabezado_linea1, encabezado_linea2, pie_linea1, pie_linea2, mostrar_marca_iados)
     VALUES (?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE tenant_id=tenant_id`, [tenantId, 'Restaurante Demo iaDoS', 'Av. Principal #123, Centro', 'Gracias por su compra', 'Desarrollado por iaDoS - iados.mx', true]);
