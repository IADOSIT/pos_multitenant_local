...
let BackupService = BackupService_1 = class BackupService {
    ...
    async dumpDeleteInsert(table, selectSql, params, deleteWhere) {
        try {
    ...
    async realizarBackupSQL(scope, tiendaFilter) {
        ...
        let empresaId = eid;
        if (tid) {
            const [t] = await this.dataSource.query('SELECT nombre, empresa_id FROM tiendas WHERE id = ?', [tid]);
            if (t) {
                label = this.sanitizeName(t.nombre);
            ...
            if (e)
                label = this.sanitizeName(e.nombre);
        }
        ...
        if (tid && empresaId) {
            parts.push(await this.dumpDeleteInsert('tiendas', 'SELECT * FROM tiendas WHERE id = ?', [tid], `id = ${tid}`));
            parts.push(await this.dumpDeleteInsert('cajas', 'SELECT * FROM cajas WHERE tienda_id = ?', [tid], `tienda_id = ${tid}`));
            parts.push(await this.dumpDeleteInsert('mesas', 'SELECT * FROM mesas WHERE tienda_id = ?', [tid], `tienda_id = ${tid}`));
            parts.push(await this.dumpDeleteInsert('ticket_configs', 'SELECT * FROM ticket_configs WHERE tienda_id = ?', [tid], `tienda_id = ${tid}`));
            parts.push(await this.dumpDeleteInsert('gateway_configs', 'SELECT * FROM gateway_configs WHERE tienda_id = ?', [tid], `tienda_id = ${tid}`));
            parts.push(await this.dumpDeleteInsert('menu_digital_config', 'SELECT * FROM menu_digital_config WHERE tienda_id = ?', [tid], `tienda_id = ${tid}`));
            parts.push(await this.dumpDeleteInsert('categorias', 'SELECT * FROM categorias WHERE empresa_id = ?', [empresaId], `empresa_id = ${empresaId}`));
            parts.push(await this.dumpDeleteInsert('productos', 'SELECT * FROM productos WHERE empresa_id = ?', [empresaId], `empresa_id = ${empresaId}`));
            parts.push(await this.dumpDeleteInsert('producto_tienda', 'SELECT pt.* FROM producto_tienda pt JOIN productos p ON pt.producto_id = p.id WHERE p.empresa_id = ?', [empresaId], `producto_id IN (SELECT id FROM productos WHERE empresa_id = ${empresaId})`));
            parts.push(await this.dumpDeleteInsert('ventas', 'SELECT * FROM ventas WHERE tienda_id = ?', [tid], `tienda_id = ${tid}`));
            parts.push(await this.dumpDeleteInsert('venta_detalles', 'SELECT d.* FROM venta_detalles d JOIN ventas v ON d.venta_id = v.id WHERE v.tienda_id = ?', [tid], `venta_id IN (SELECT id FROM ventas WHERE tienda_id = ${tid})`));
            parts.push(await this.dumpDeleteInsert('venta_pagos', 'SELECT p.* FROM venta_pagos p JOIN ventas v ON p.venta_id = v.id WHERE v.tienda_id = ?', [tid], `venta_id IN (SELECT id FROM ventas WHERE tienda_id = ${tid})`));
            parts.push(await this.dumpDeleteInsert('pedidos', 'SELECT * FROM pedidos WHERE tienda_id = ?', [tid], `tienda_id = ${tid}`));
            parts.push(await this.dumpDeleteInsert('pedido_detalles', 'SELECT d.* FROM pedido_detalles d JOIN pedidos p ON d.pedido_id = p.id WHERE p.tienda_id = ?', [tid], `pedido_id IN (SELECT id FROM pedidos WHERE tienda_id = ${tid})`));
            parts.push(await this.dumpDeleteInsert('movimientos_caja', 'SELECT mc.* FROM movimientos_caja mc JOIN cajas c ON mc.caja_id = c.id WHERE c.tienda_id = ?', [tid], `caja_id IN (SELECT id FROM cajas WHERE tienda_id = ${tid})`));
            parts.push(await this.dumpDeleteInsert('movimientos_inventario', 'SELECT * FROM movimientos_inventario WHERE tienda_id = ?', [tid], `tienda_id = ${tid}`));
            parts.push(await this.dumpDeleteInsert('encuestas_servicio', 'SELECT es.* FROM encuestas_servicio es JOIN pedidos p ON es.pedido_id = p.id WHERE p.tienda_id = ?', [tid], `pedido_id IN (SELECT id FROM pedidos WHERE tienda_id = ${tid})`));
        }
        else if (eid) {
            parts.push(await this.dumpDeleteInsert('tiendas', 'SELECT * FROM tiendas WHERE empresa_id = ?', [eid], `empresa_id = ${eid}`));
            parts.push(await this.dumpDeleteInsert('cajas', 'SELECT c.* FROM cajas c JOIN tiendas t ON c.tienda_id = t.id WHERE t.empresa_id = ?', [eid], `tienda_id IN (SELECT id FROM tiendas WHERE empresa_id = ${eid})`));
            parts.push(await this.dumpDeleteInsert('mesas', 'SELECT m.* FROM mesas m JOIN tiendas t ON m.tienda_id = t.id WHERE t.empresa_id = ?', [eid], `tienda_id IN (SELECT id FROM tiendas WHERE empresa_id = ${eid})`));
            parts.push(await this.dumpDeleteInsert('ticket_configs', 'SELECT tc.* FROM ticket_configs tc JOIN tiendas t ON tc.tienda_id = t.id WHERE t.empresa_id = ?', [eid], `tienda_id IN (SELECT id FROM tiendas WHERE empresa_id = ${eid})`));
            parts.push(await this.dumpDeleteInsert('gateway_configs', 'SELECT gc.* FROM gateway_configs gc JOIN tiendas t ON gc.tienda_id = t.id WHERE t.empresa_id = ?', [eid], `tienda_id IN (SELECT id FROM tiendas WHERE empresa_id = ${eid})`));
            parts.push(await this.dumpDeleteInsert('menu_digital_config', 'SELECT mc.* FROM menu_digital_config mc JOIN tiendas t ON mc.tienda_id = t.id WHERE t.empresa_id = ?', [eid], `tienda_id IN (SELECT id FROM tiendas WHERE empresa_id = ${eid})`));
            parts.push(await this.dumpDeleteInsert('categorias', 'SELECT * FROM categorias WHERE empresa_id = ?', [eid], `empresa_id = ${eid}`));
            parts.push(await this.dumpDeleteInsert('productos', 'SELECT * FROM productos WHERE empresa_id = ?', [eid], `empresa_id = ${eid}`));
            parts.push(await this.dumpDeleteInsert('producto_tienda', 'SELECT pt.* FROM producto_tienda pt JOIN productos p ON pt.producto_id = p.id WHERE p.empresa_id = ?', [eid], `producto_id IN (SELECT id FROM productos WHERE empresa_id = ${eid})`));
            parts.push(await this.dumpDeleteInsert('ventas', 'SELECT v.* FROM ventas v JOIN tiendas t ON v.tienda_id = t.id WHERE t.empresa_id = ?', [eid], `tienda_id IN (SELECT id FROM tiendas WHERE empresa_id = ${eid})`));
            parts.push(await this.dumpDeleteInsert('venta_detalles', 'SELECT d.* FROM venta_detalles d JOIN ventas v ON d.venta_id = v.id JOIN tiendas t ON v.tienda_id = t.id WHERE t.empresa_id = ?', [eid], `venta_id IN (SELECT id FROM ventas v2 JOIN tiendas t ON v2.tienda_id = t.id WHERE t.empresa_id = ${eid})`));
            parts.push(await this.dumpDeleteInsert('venta_pagos', 'SELECT p.* FROM venta_pagos p JOIN ventas v ON p.venta_id = v.id JOIN tiendas t ON v.tienda_id = t.id WHERE t.empresa_id = ?', [eid], `venta_id IN (SELECT id FROM ventas v2 JOIN tiendas t ON v2.tienda_id = t.id WHERE t.empresa_id = ${eid})`));
            parts.push(await this.dumpDeleteInsert('pedidos', 'SELECT p.* FROM pedidos p JOIN tiendas t ON p.tienda_id = t.id WHERE t.empresa_id = ?', [eid], `tienda_id IN (SELECT id FROM tiendas WHERE empresa_id = ${eid})`));
            parts.push(await this.dumpDeleteInsert('pedido_detalles', 'SELECT d.* FROM pedido_detalles d JOIN pedidos p ON d.pedido_id = p.id JOIN tiendas t ON p.tienda_id = t.id WHERE t.empresa_id = ?', [eid], `pedido_id IN (SELECT id FROM pedidos p2 JOIN tiendas t ON p2.tienda_id = t.id WHERE t.empresa_id = ${eid})`));
            parts.push(await this.dumpDeleteInsert('movimientos_caja', 'SELECT mc.* FROM movimientos_caja mc JOIN cajas c ON mc.caja_id = c.id JOIN tiendas t ON c.tienda_id = t.id WHERE t.empresa_id = ?', [eid], `caja_id IN (SELECT id FROM cajas c2 JOIN tiendas t ON c2.tienda_id = t.id WHERE t.empresa_id = ${eid})`));
            parts.push(await this.dumpDeleteInsert('movimientos_inventario', 'SELECT mi.* FROM movimientos_inventario mi JOIN tiendas t ON mi.tienda_id = t.id WHERE t.empresa_id = ?', [eid], `tienda_id IN (SELECT id FROM tiendas WHERE empresa_id = ${eid})`));
            parts.push(await this.dumpDeleteInsert('encuestas_servicio', 'SELECT es.* FROM encuestas_servicio es JOIN pedidos p ON es.pedido_id = p.id JOIN tiendas t ON p.tienda_id = t.id WHERE t.empresa_id = ?', [eid], `pedido_id IN (SELECT id FROM pedidos p2 JOIN tiendas t ON p2.tienda_id = t.id WHERE t.empresa_id = ${eid})`));
        }
    ...
    fbSignal(ms = 7000) { return AbortSignal.timeout(ms); }
    async fileBrowserLogin(config) {
        ...
        if (!res.ok)
            throw new Error(`Login FileBrowser fallido (${res.status})`);
        return res.text();
    }
    async fileBrowserMkdir(base, token, remotePath) {
    ...
    async uploadViaFileBrowser(config, localFile, filename) {
        let base = (config.sftp_host || '').trim().replace(/\/$/, '');
        if (base && !base.startsWith('http'))
            base = `https://${base}`;
        ...
        if (!res.ok)
            throw new Error(`FileBrowser upload fallido (${res.status}): ${await res.text()}`);
    ...
    async validarBackup(filename) {
        const safe = path.basename(filename);
        if (!safe.endsWith('.sql'))
            return { ok: false, info: null, error: 'Solo archivos .sql' };
        const filepath = path.join(this.backupsDir, safe);
        if (!fs.existsSync(filepath))
            return { ok: false, info: null, error: 'Archivo no encontrado' };
        const content = fs.readFileSync(filepath, 'utf8');
        const lines = content.split('\n').slice(0, 10);
        const meta = { archivo: safe };
        for (const line of lines) {
            const m1 = line.match(/^-- TIENDA: (.+)$/);
            if (m1)
                meta.tienda_nombre = m1[1].trim();
            const m2 = line.match(/^-- TIENDA_ID: (.+)$/);
            if (m2)
                meta.tienda_id = m2[1].trim();
            const m3 = line.match(/^-- EMPRESA_ID: (.+)$/);
            if (m3)
                meta.empresa_id = m3[1].trim();
            const m4 = line.match(/^-- FECHA: (.+)$/);
            if (m4)
                meta.fecha_backup = m4[1].trim();
        }
        ...
        if (meta.tienda_id && meta.tienda_id !== 'null') {
            try {
                const [t] = await this.dataSource.query('SELECT nombre FROM tiendas WHERE id = ?', [parseInt(meta.tienda_id)]);
                meta.tienda_actual = t?.nombre || '⚠️ Tienda no encontrada en BD actual';
            }
            catch {
                meta.tienda_actual = 'Error verificando tienda';
            }
        }
