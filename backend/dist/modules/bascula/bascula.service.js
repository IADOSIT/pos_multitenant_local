...
let BasculaService = class BasculaService {
    ...
    async updateConfig(tiendaId, dto, scope) {
        ...
        const allowed = [
            'activo', 'printer_ip', 'printer_port', 'label_width_mm', 'label_height_mm',
    ...
    async registrarPesaje(dto, scope) {
        ...
        const [producto] = await this.dataSource.query(`SELECT id, nombre, precio, tenant_id, empresa_id FROM productos WHERE id = ?`, [dto.producto_id]);
        if (!producto)
