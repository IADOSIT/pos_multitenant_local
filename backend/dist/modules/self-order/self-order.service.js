...
let SelfOrderService = class SelfOrderService {
    ...
    async getTiendaPublica(tienda_id, mesa_numero) {
        ...
        const [tienda] = await this.dataSource.query(`SELECT t.nombre, t.config_pos, e.nombre as empresa_nombre, e.logo_url
       FROM tiendas t JOIN empresas e ON t.empresa_id = e.id
       WHERE t.id = ? AND t.activo = 1`, [tienda_id]);
        if (!tienda)
        ...
        if (!configPos?.self_order_enabled)
            throw new common_1.BadRequestException('Self Order no está activo en esta tienda');
        return {
    ...
    async getMenuPublico(tienda_id) {
        const categorias = await this.dataSource.query(`SELECT c.id, c.nombre, c.color, c.icono, c.imagen_url, c.orden
       FROM categorias c
       INNER JOIN tiendas t ON t.empresa_id = c.empresa_id
       WHERE t.id = ? AND c.activo = 1
       ORDER BY c.orden ASC`, [tienda_id]);
        const productos = await this.dataSource.query(`SELECT p.id, p.nombre, p.descripcion, p.precio, p.imagen_url, p.categoria_id, p.impuesto_pct, p.unidad
       FROM productos p
       ...
       WHERE t.id = ? LIMIT 1`, [tienda_id]);
        ...
        const campos_formulario = (0, campos_formulario_helper_1.resolveCamposFormulario)(cfg);
        return {
            categorias,
    ...
    async crearPedidoCliente(tienda_id, mesa_numero, body) {
        ...
        if (!mesa)
            throw new common_1.NotFoundException('Mesa no encontrada');
        const [tienda] = await this.dataSource.query(`SELECT tenant_id, empresa_id, config_pos FROM tiendas WHERE id = ?`, [tienda_id]);
        if (!tienda)
            throw new common_1.NotFoundException('Tienda no encontrada');
        const configPos = typeof tienda.config_pos === 'string' ? JSON.parse(tienda.config_pos) : tienda.config_pos;
        if (!configPos?.self_order_enabled)
            throw new common_1.BadRequestException('Self Order no está activo');
        if (!body.items?.length)
            throw new common_1.BadRequestException('El pedido está vacío');
        const [empresaCfgRow] = await this.dataSource.query(`SELECT config_especial FROM empresas WHERE id = ? LIMIT 1`, [tienda.empresa_id]);
        const camposConfig = (0, campos_formulario_helper_1.resolveCamposFormulario)(empresaCfgRow?.config_especial);
        const CAMPO_A_BODY = {
        ...
        const encuesta_token = (0, uuid_1.v4)();
        ...
        const saved = (await this.pedidoRepo.save(this.pedidoRepo.create(pedidoData)));
    ...
    async getEstadoPedido(encuesta_token) {
    ...
    async confirmarPedidoMesero(pedido_id, scope) {
        const pedido = await this.pedidoRepo.findOne({ where: { id: pedido_id, tienda_id: scope.tienda_id } });
        if (!pedido)
            throw new common_1.NotFoundException('Pedido no encontrado');
        if (!pedido.self_order)
            throw new common_1.BadRequestException('No es un pedido self-order');
    ...
    async responderEncuesta(encuesta_token, data) {
        const pedido = await this.pedidoRepo.findOne({ where: { encuesta_token } });
        if (!pedido)
            throw new common_1.NotFoundException('Token inválido');
        if (!pedido.venta_id)
            throw new common_1.BadRequestException('El pago aún no se ha procesado');
    ...
    async getKPIs(scope, desde, hasta) {
        ...
        const dateParams = hasDate ? [desde, `${hasta} 23:59:59`] : [];
    ...
    `, [scope.tienda_id, ...dateParams]);
        ...
        const totalEncuestas = await this.dataSource.query(`SELECT COUNT(*) as total FROM encuestas_servicio WHERE tienda_id = ? ${encDateClause}`, [scope.tienda_id, ...dateParams]);
    ...
    async getTiendaPublicaBySlug(slug, mesa_numero) {
        const rows = await this.dataSource.query('SELECT id FROM tiendas WHERE slug = ? AND activo = 1', [slug]);
        if (!rows.length)
            throw new common_1.NotFoundException('Tienda no encontrada');
        return this.getTiendaPublica(rows[0].id, mesa_numero);
    }
    async getMenuPublicoBySlug(slug) {
        const rows = await this.dataSource.query('SELECT id FROM tiendas WHERE slug = ? AND activo = 1', [slug]);
        if (!rows.length)
            throw new common_1.NotFoundException('Tienda no encontrada');
        return this.getMenuPublico(rows[0].id);
    }
    async crearPedidoBySlug(slug, mesa_numero, body) {
        const rows = await this.dataSource.query('SELECT id FROM tiendas WHERE slug = ? AND activo = 1', [slug]);
        if (!rows.length)
            throw new common_1.NotFoundException('Tienda no encontrada');
        return this.crearPedidoCliente(rows[0].id, mesa_numero, body);
    }
    async getQRPrintable(tienda_id, mesa_id, baseUrl) {
        const mesa = await this.mesaRepo.findOne({ where: { id: mesa_id, tienda_id, activo: true } });
        if (!mesa)
            throw new common_1.NotFoundException('Mesa no encontrada');
        const [tienda] = await this.dataSource.query(`SELECT t.nombre, t.config_pos, e.nombre as empresa_nombre, e.logo_url
       FROM tiendas t JOIN empresas e ON t.empresa_id = e.id WHERE t.id = ?`, [tienda_id]);
        let configPos = {};
        try {
            configPos = tienda?.config_pos ? JSON.parse(tienda.config_pos) : {};
        }
        catch {
            configPos = {};
        }
...
exports.SelfOrderService = SelfOrderService = __decorate([
