...
let DevolucionesService = class DevolucionesService {
    ...
    async generateFolio(tienda_id) {
        return this.dataSource.transaction(async (manager) => {
            const [tienda] = await manager.query('SELECT folio_devolucion_counter FROM tiendas WHERE id = ? FOR UPDATE', [tienda_id]);
            const newCounter = (tienda?.folio_devolucion_counter || 0) + 1;
            await manager.query('UPDATE tiendas SET folio_devolucion_counter = ? WHERE id = ?', [newCounter, tienda_id]);
    ...
    async findByVenta(ventaId, scope) {
        try {
            ...
            throw new common_1.InternalServerErrorException(`Error consultando devoluciones: ${e?.message}`);
        }
    }
    ...
    async crear(dto, scope) {
        ...
        let itemsDevolucion = [];
        try {
            [venta] = await this.dataSource.query(`SELECT v.id, v.folio, v.estado, v.tenant_id, v.tienda_id
         FROM ventas v WHERE v.id = ? AND v.tenant_id = ?`, [dto.venta_id ?? null, scope.tenant_id ?? null]);
            if (!venta)
         ...
         FROM venta_detalles vd WHERE vd.venta_id = ?`, [dto.venta_id ?? null]);
            const devueltoMap = {};
            ...
            throw new common_1.InternalServerErrorException(`Error al procesar devolución: ${e?.message}`);
        }
        ...
        try {
            const [cajaAbierta] = await this.dataSource.query(`SELECT id, total_salidas FROM cajas WHERE tienda_id = ? AND estado = 'abierta' LIMIT 1`, [scope.tienda_id]);
            if (cajaAbierta) {
                const concepto = `Devolución ${devolucion.folio} (Venta ${venta.folio})`.slice(0, 200);
                await this.dataSource.query(`INSERT INTO movimientos_caja (caja_id, usuario_id, tipo, monto, concepto)
           VALUES (?, ?, 'salida', ?, ?)`, [cajaAbierta.id, scope.id || scope.sub, montoTotal, concepto]);
                await this.dataSource.query(`UPDATE cajas SET total_salidas = total_salidas + ? WHERE id = ?`, [montoTotal, cajaAbierta.id]);
