...
let LicenciasService = class LicenciasService {
    ...
    applyPayload(lic, payload, machineFingerprint) {
        lic.codigo_activacion = this.encrypt(payload);
        lic.plan = payload.p;
        ...
        if (lic.machine_locked && machineFingerprint) {
            if (lic.machine_fingerprint && lic.machine_fingerprint !== machineFingerprint) {
                throw new common_1.BadRequestException('Esta licencia ya está vinculada a otro equipo. Contacta al administrador.');
            }
            lic.machine_fingerprint = machineFingerprint;
        }
    }
    async activar(tenantId, codigoFormateado, machineFingerprint) {
    ...
    async generarToken(licenciaId, activationCode) {
    ...
    async activarConToken(token, machineFingerprint) {
        ...
        if (!lic.activation_token_expires || new Date() > lic.activation_token_expires) {
            throw new common_1.BadRequestException('El enlace de activación ha expirado (válido 48 h)');
        }
    ...
    async refreshEstado(lic) {
        if (lic.estado === licencia_entity_1.LicenciaEstado.SUSPENDIDA)
            return lic;
        if (lic.permanente) {
            if (lic.estado !== licencia_entity_1.LicenciaEstado.ACTIVA) {
                lic.estado = licencia_entity_1.LicenciaEstado.ACTIVA;
                await this.repo.save(lic);
            }
            return lic;
    ...
    async getEstado(tenantId) {
        ...
        const bloqueada = !lic.permanente && expirada && lic.estado !== licencia_entity_1.LicenciaEstado.SUSPENDIDA;
