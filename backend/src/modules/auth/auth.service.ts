...
export class AuthService {
  ...
  private async findOneWithRetry<T extends object>(repo: Repository<T>, opts: object, ctx: string): Promise<T | null> {
    try {
      return await (repo as any).findOne(opts);
    } catch (err) {
  ...
  async loginPin(pin: string, tienda_id: number, user_id?: number) {
    ...
    let empresa2: Empresa | null = null;
    try {
  ...
  async verifyPin(pin: string, tienda_id: number) {
    const user = await this.findOneWithRetry(
      this.usersRepo,
