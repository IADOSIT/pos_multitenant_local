@extends('layouts.admin', ['title'=>'Vista previa de importación','header'=>'Vista previa de importación'])
...
<div class="max-w-6xl">
    ...
    <div class="bg-white rounded-xl shadow-sm border p-4">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-sm text-gray-500">Válidas</p>
        </div>
      </div>
    </div>
  ...
  <div class="bg-white rounded-xl shadow-sm border overflow-hidden">
    <div class="p-4 border-b">
      ...
      <p class="text-sm text-gray-500">Revisa los datos antes de importar</p>
    </div>
    ...
    <div class="overflow-x-auto">
      <table class="w-full text-sm">
        <thead class="bg-gray-50">
          <tr>
            <th class="px-4 py-3 text-left font-medium text-gray-600">Línea</th>
        ...
        <tbody class="divide-y">
          @foreach($rows as $row)
            <tr class="{{ !empty($row['_errors']) ? 'bg-red-50' : '' }}">
              ...
              <td class="px-4 py-3">${{ number_format((float)($row['precio'] ?? 0), 2) }}</td>
              <td class="px-4 py-3">
                @php
                  $catName = $row['categoria_nombre'] ?? '';
                  $catFound = false;
                ...
                @endphp
                <span class="{{ $catName && !$catFound ? 'text-orange-600' : '' }}">
                  {{ $catName ?: '-' }}
                  @if($catName && !$catFound)
                    <span class="text-xs">(no encontrada)</span>
                  @endif
                </span>
              </td>
              <td class="px-4 py-3">
  ...
  <div class="mt-6 flex items-center justify-between">
    <a href="{{ route('admin.import-export.productos') }}" class="text-gray-600 hover:text-gray-800 font-medium flex items-center gap-1">
    ...
    @if(count($validRows) > 0)
      <form method="POST" action="{{ route('admin.import-export.productos.import') }}">
        @csrf
        <button type="submit" class="inline-flex items-center gap-2 bg-primary-600 hover:bg-primary-700 text-white px-6 py-2.5 rounded-lg font-medium transition">
          ...
          Importar {{ count($validRows) }} productos
        </button>
      </form>
    @else
      <p class="text-red-600 font-medium">No hay filas válidas para importar</p>
    @endif
  </div>
</div>
@endsection
