@extends('layouts.admin', ['title'=>'Vista previa - Inventario','header'=>'Vista previa de importacion'])
...
<div class="max-w-6xl">
        ...
        <tbody class="divide-y">
          @foreach($rows as $row)
            <tr class="{{ !empty($row['_errors']) ? 'bg-red-50' : '' }}">
              ...
              <td class="px-4 py-3 text-right font-bold">{{ $row['stock'] ?? '-' }}</td>
              <td class="px-4 py-3">
                @if(!empty($row['_errors']))
  ...
  <div class="mt-6 flex items-center justify-between">
    <a href="{{ route('admin.import-export.inventario') }}" class="text-gray-600 hover:text-gray-800 font-medium flex items-center gap-1">
    ...
    @if(count($validRows) > 0)
      <form method="POST" action="{{ route('admin.import-export.inventario.import') }}">
        @csrf
        <button type="submit" class="inline-flex items-center gap-2 bg-primary-600 hover:bg-primary-700 text-white px-6 py-2.5 rounded-lg font-medium transition">
          ...
          Importar {{ count($validRows) }} registros
        </button>
      </form>
    @else
