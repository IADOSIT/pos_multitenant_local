@extends('layouts.admin', ['title' => 'Editar Cliente', 'header' => 'Editar Cliente'])

@section('content')
<div class="max-w-2xl mx-auto">
    <form action="{{ route('admin.clientes.update', $cliente->id) }}" method="POST" class="bg-white rounded-xl shadow-sm border p-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-5">
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="whatsapp" id="whatsapp" value="{{ old('whatsapp', $cliente->whatsapp) }}"
            ...
            <div>
                ...
                <textarea name="direccion" id="direccion" rows="2"
                          ...
                          placeholder="Direccion de entrega">{{ old('direccion', $cliente->direccion) }}</textarea>
                @error('direccion')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="notas" id="notas" rows="3"
                          ...
                          placeholder="Notas adicionales sobre el cliente">{{ old('notas', $cliente->notas) }}</textarea>
                @error('notas')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <input type="checkbox" name="enviar_estatus" id="enviar_estatus" value="1"
                       {{ old('enviar_estatus', $cliente->enviar_estatus) ? 'checked' : '' }}
        ...
        <div class="flex items-center justify-between mt-8 pt-6 border-t">
            ...
            <div class="flex gap-3">
                <button type="button" onclick="document.getElementById('delete-form').submit()"
                        class="px-4 py-2.5 text-red-600 hover:bg-red-50 rounded-lg font-medium">
                    Eliminar
                </button>
    ...
    <form id="delete-form" action="{{ route('admin.clientes.destroy', $cliente->id) }}" method="POST" class="hidden">
        @csrf
        @method('DELETE')
    </form>
</div>
