...
let SelfOrderService = class SelfOrderService {
    ...
    `, [scope.tienda_id]);
        ...
        const totalEncuestas = await this.dataSource.query(`SELECT COUNT(*) as total FROM encuestas_servicio WHERE tienda_id = ? ${desde && hasta ? `AND created_at BETWEEN '${desde}' AND '${hasta} 23:59:59'` : ''}`, [scope.tienda_id]);
