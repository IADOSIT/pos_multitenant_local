    ...
    if ($checkResult -match "^0$" -or $checkResult -match "doesn't exist" -or $LASTEXITCODE -ne 0) {
        ...
        if (Test-Path $seedFile) {
            ...
            if ($hashOut -match '^\$2[ab]\$') {
                ...
                $updateSql  = "UPDATE users SET password='$hashAdmin'  WHERE email IN ('admin@iados.mx','admin2@iados.mx');" +
                              "UPDATE users SET password='$hashCajero' WHERE email IN ('cajero@iados.mx','mesero@iados.mx');"
                $ErrorActionPreference = "SilentlyContinue"
