...
Write-Host ""
Write-Host "  POS-iaDoS Parche v2.2.42" -ForegroundColor Cyan
Write-Host ""
...
if (Test-Path $envFile) {
    ...
    if ($envContent -match 'APP_VERSION=') {
        $envContent = $envContent -replace 'APP_VERSION=.*', 'APP_VERSION=2.2.42'
    } else {
...
"
    ...
    Write-Host "    OK: APP_VERSION=2.2.42 en .env" -ForegroundColor Green
}
...
if (Test-Path $vjFile) {
    ...
    Write-Host "    OK: version.json -> 2.2.42" -ForegroundColor Green
}
...
Write-Host ""
if ($ok) {
    Write-Host "  PARCHE v2.2.42 APLICADO Y BACKEND OK" -ForegroundColor Green
} else {
